...
onWebLoad(function () {
  onWebLoad.loaded = true;
});
//onload=function(){ pageInit();}
function isReady() {
  return onWebLoad.loaded;
}
var $id = function (o) {
  return document.getElementById(o);
};
var cid = function (fid, id) {
  var _div = document.createElement('div');
  _div.id = id;
  $id(fid).appendChild(_div);
  return _div;
};
var swfstr = window.swfstr = function (id, url, width, height, basev, flashvars) {
  var obj = '';
  ...
  obj += '<embed flashvars="' + flashvars + '" id="' + id + '_ff" name="' + id + '_ff" base="' + basev + '" src="' + url + '" quality="high" wmode="transparent" width="' + width + '" height="' + height + '" allowfullscreen="true" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />';
  obj += '</object>';
  return obj;
}
function insertSwf(elm, url, width, height, base, flashvars) {
  if (!$id(elm))return;
  ...
  if (fls.f) {
    if (fls.v >= 10) {
      if (!base)base = "";
      $id(elm).innerHTML = swfstr(id, url, width, height, base, flashvars);
...
var FlashManager = function (opt) {
  ...
  this.init = function () {
    ...
    insertSwf(_this.option.contentId + '_content', _this.option.flashUrl, _this.option.width, _this.option.height, _this.option.base, flashvars)
    {//初始化flash里的方法
      ...
      window['SWFCallJSToStart' + _this.objectID()] = function (num) {
        //alert("CallJS")
        if (typeof(_this.option.onClickRollEvent) == 'function') {
          setTimeout(function () {
            _this.option.onClickRollEvent(num)
          }, 50);
        }
      };
      //3回调函数。
      window['SWFPlayComplete' + _this.objectID()] = function (num) {
        if (typeof(_this.option.onCompleteRollEvent) == 'function') {
          setTimeout(function () {
            _this.option.onCompleteRollEvent(num)
          }, 100);
        }
      };
      //alert(this.objectID())
...
//取得该实例化对象。
FlashManager.init = function (opt) {
  return new FlashManager(opt);
};
